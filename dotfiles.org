...
#+begin_src lua :tangle :tangle (when (eq system-type 'darwin) "~/.hammerspoon/modules/dashboard.lua")
    ...
    function generate_result()
       local git_status = {
          org = { 'cd ~/Documents/org && git status  --porcelain', "col-git-status", 5 },
          roam = { "cd ~/Documents/roam && git status  --porcelain", "col-git-status", 12 },
          dotfiles = { "cd ~/.emacs.d/ && git status  --porcelain", "col-git-status", 7 }
       }
    ...
    function getFiles(path)
       local ret = {}
       local result = ''
       local output = io.popen("git status | cat && sleep 10")
