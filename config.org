...
I can use ~C-c I~ to open this config file and use ~C-c R~ to reload it.
By the way, I can use ~C-c i~ to open system config file.
...
#+end_src
** Install packages
~straight.el~ is a good tool to manage packages, but it is slow when pull all the packages,
...
** Wgrep search result
use ~consult-grep~ to get global search result, ~C-S-a~ (embark-act) and ~E~ to export and ~C-c C-p~ to use wgrep to edit, after saved, use ~wgrep-save-all-buffers~ to confirm changes.
#+begin_src emacs-lisp
  (use-package embark
    :straight t
    :bind
...
#+end_src
*** HTML preview
~M-x httpd-start~ first to start server
then ~M-x impatient-mode~
visit http://localhost:8080/imp/
...
use ~C-c C-c~ to toggle finish checkbox
~M-S-RET~ to create new checkbox
...
Use ~C-c |~ to fire create table function
input ~|||~ and press TAB to quick insert a three colum table
** Agenda
...
for project search, use ~find-name-dired~ select root folder path and ~wdired-change-to-wdired-mode~
...
#+begin_src emacs-lisp
  (use-package chatgpt-shell
      :straight
      (:host github :repo "xenodium/chatgpt-shell" :branch "main"))
...
#+begin_src emacs-lisp
  ...
  ;; UI start ---------------------------------------
  (defun hikarian-create-controller()
    "Prompt user to enter a controller name"
    (interactive)

    (let ((input (read-string "Enter controller name:")))
      (setq hikarian-resource-name input)
      (setq hikarian-resource-type "controller")

      (message "you want to create controller (single) %s" input)
      (let* (
             (name-arrayed (split-string input "/"))
             (namespace (string-join (butlast name-arrayed) "/"))
             (controller-name (car (last name-arrayed)))
             )

        (hikarian-find-or-create-file
         (concat "app/controllers/" controller-name "s_controller.rb")
         hikarian-resource-name))))

  (defun hikarian-create-model()
    "Prompt user to enter a model name"
    (interactive)

    (let ((input (read-string "Enter model name:")))
      (let ((input (read-string "Enter model name:")))
        (setq hikarian-resource-name input)
        (setq hikarian-resource-type "model")
        (message "you want to create model: %s" input)
        (let* (
               (name-arrayed (split-string input "/"))
               (namespace (string-join (butlast name-arrayed) "/"))
               (model-name (car (last name-arrayed)))
               )

          (hikarian-find-or-create-file
           (concat "app/models/" model-name ".rb")
           hikarian-resource-name)))))
  ...
  (defun hikarian-get-project-root ()
    "get project root depend on user config"
    "~/code/gfresh/beachpricex")

  (defun hikarian-find-or-create-file (full_path name-with-space)
    "if file exist open it else create"
    (let ((project-path-file (concat (hikarian-get-project-root) "/" full_path)))
      (message "will check %s" project-path-file)
      (message "is file exists? %s" (file-exists-p project-path-file))
      (if (file-exists-p project-path-file)
          (find-file project-path-file)
        (hikarian-init-file project-path-file name-with-space))))

  (defun hikarian-init-file (project-path-file name-with-space)
    (message "will create file with some content .....%s ... type is: %s" project-path-file hikarian-resource-type)
    (find-file project-path-file)
    (insert (yas-expand-snippet (yas-lookup-snippet (concat "hikarian_" hikarian-resource-type)))))

  (defun hikarian-single-resource-name ()
    (car (last (split-string hikarian-resource-name "/"))))

  (defun hikarian-capitalize-single-resource-name ()
    (capitalize (hikarian-single-resource-name)))

  (defun hikarian-pluralize-resource-name ()
    (inflection-pluralize-string (hikarian-single-resource-name)))

  (defun hikarian-capitalize-pluralize-resource-name ()
    (capitalize (hikarian-pluralize-resource-name)))

  (defun hikarian-namespace-name ()
    (if (eq 1 (length (split-string hikarian-resource-name "/")))
        nil
      (car (split-string hikarian-resource-name "/"))))

  (defun hikarian-resource-full-class-name ()
    (if (hikarian-namespace-name)
        (concat (capitalize (hikarian-namespace-name)) "::" (hikarian-capitalize-single-resource-name))
      (hikarian-capitalize-single-resource-name)))


  (defun hikarian-resource-full-class-pluralize-name ()
    (if (hikarian-namespace-name)
        (concat (capitalize (hikarian-namespace-name)) "::" (hikarian-capitalize-pluralize-resource-name))
      (hikarian-capitalize-pluralize-resource-name)))

  (defun hikarian-get-current-resource ()
    ;; get project path of current file
    (interactive)
    ;; controllers/books_controller.rb
    ;; model/book.rb
    ;; views/admin/new.html.erb
    ;; TODO arch/entity ...
    ;; (file-relative-name "~/src/dotfiles/emacs.d/conf.org" "~/src/dotfiles")
    (buffer-file-name)
    (let ((current_file_path (file-relative-name (buffer-file-name) (hikarian-get-project-root))))
      (message current_file_path)
      (cond
       ((string-match-p "controller" current_file_path)
        (hikarian-extract-current-resource (car (last (split-string current_file_path "/"))) "controller"))
       ((string-match-p "model"  current_file_path)
        (hikarian-extract-current-resource (car (last (split-string current_file_path "/"))) "model"))
       ((string-match-p "view"  current_file_path)
        (hikarian-extract-current-resource (car (last (split-string current_file_path "/"))) "view"))
       )))
  ;;(car (last '(1 2 3)))
  ;;(string-match-p "controller" "app/controllers/abc_controller.rb")

  ;; e.g abc_def_controller aaa.rb bbb(view)
  ;; return the resource name
  (defun hikarian-extract-current-resource(resource_file_name type)
    ;;(message "the params is %s %s"  resource_file_name type)  ;; the params is app/models/bid.rb model
    (let ((result (cond
                   ((equal "controller" type) (car (split-string resource_file_name  "_")))
                   ((equal "model" type) (car (split-string resource_file_name  "\\.")))
                   ((equal "view" type) ()) ;; TODO app/views/users/index.html remove app/views (users/index) get first
                   )))

      (hikarian-pick-file-of-resource result)
      ))

  ;; (car (last (split-string "app/models/bid.rb" "/")))
  ;; (car (split-string "bid.rb" "\\."))
  ;; (car (split-string "app/models/bid_ddd.rb" "_"))

  ;;(setq hikarian-resource-name "admin/user")
  ;;(hikarian-namespace-name)
  ;;(hikarian-resource-full-class-name)

  (provide 'hikarian)
             ;;; hikarian.el ends here

  (defun hikarian-related-files-of-resource (resource)
    "Prompt user to pick a choice from a list accourding resource"
    (interactive)
    (let ((related-files (hikarian-list-related-files resource)))
      (let ((related-file-with-status (hikarian-list-related-files-with-status related-files)))

       (let ((choices related-file-with-status))

         (alist-get
          (completing-read "Choose: " choices)
          choices nil nil 'equal))
          )))
  ;;      (completing-read "Open or create file:" choices )))

  (defun hikarian-pick-file-of-resource(resource)
    ;;(interactive)
    (let ((result (hikarian-related-files-of-resource resource)))
      (message "the result is %s" result)
      (find-file (concat (hikarian-get-project-root) "/" result))
      ))

  (defun hikarian-list-related-files (resource)
    (list
     (concat "app/controllers/" resource "s_controller.rb")
     (concat "app/models/" resource ".rb")
     (concat "app/views/" resource "s/index.html.erb")
     (concat "app/views/" resource "s/show.html.erb")
     (concat "app/views/" resource "s/edit.html.erb") ;; TODO dymatic list view files
     )
    )

  (defun hikarian-fixed-resource-files (resource)
    (list
     (concat "app/controllers/" resource "s_controller.rb")
     (concat "app/models/" resource ".rb")
     (concat "app/views/" resource "s/index.html.erb")
  ...
  (defun hikarian-exists-resource-files (resource)
    ;; TODO list files of app/views/resources/**
    )

  (defun hikarian-detect-file-exist (file_path)
    "check file exists in project"
    (let ((project-path-file (concat (hikarian-get-project-root) "/" file_path)))
      (file-exists-p project-path-file)))


  ;; TODO return relation resource showing if file exists
  ;; https://stackoverflow.com/questions/35390729/how-to-return-the-value-instead-of-key-by-completing-read

  (defun hikarian-list-related-files-with-status (relate-files)
    (message "here")
    (message "%s" relate-files)
    (mapcar (lambda (file)
              (if (hikarian-detect-file-exist file)
                  (cons file file)
                (cons (concat file " (new)") file)))
            relate-files))
#+end_src
