...
#+begin_src emacs-lisp
  ...
  (defun hikarian-extract-current-resource(resource_file_name type)
    (cond
     ((eq "controller" type) (car split-string resource_file_name  "_"))
     ((eq "model" type) (car split-string resource_file_name  "."))
     ((eq "view" type) ()) ;; TODO app/views/users/index.html remove app/views (users/index) get first
  ))
  ...
  (defun hikarian-related-files-of-resource ()
    "Prompt user to pick a choice from a list accourding resource"
    (interactive)
    (let ((choices '("cat" "dog" "dragon" "tiger")))
       (completing-read "Open bookmark:" choices )))


  (defun hikarian-pick-file-of-resource()
    (interactive)
    (let ((result (hikarian-related-files-of-resource)))
