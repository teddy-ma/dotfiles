...
#+begin_src emacs-lisp
    ...
    (defun teddy-ma/toggle-mode-line-modes(@n)
      ...
      (let (
        ...
        (message "idx is %s" idx )
        (message "index is %s" teddy-ma/current-mode-line-index)
        )

        ;;    (setq teddy-ma/mode-line-content (eval `mode-line-modes))
      ;;(setq teddy-ma/mode-line-content (teddy-ma/render-cyan))

      (setq teddy-ma/current-mode-line-index (+ teddy-ma/current-mode-line-index 1))
      (teddy-ma/refresh-mode-line)
      )

    (eql (eval (nth 1 teddy-ma/mode-line-left-content)) (eval `mode-line-modes))

    (message "%s" teddy-ma/mode-line-content)
