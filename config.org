...
pull all package in command line: ~(async-shell-command "emacs -batch -l ~/.emacs.d/init.el -f straight-pull-all")~
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (when (eq system-type 'windows-nt) (set-face-attribute 'default nil :height 150))
#+end_src
...
** nested check list
- [] C-c [/]
ctrl+shift+enter create new checkbox
...
https://xenodium.com/emacs-clone-git-repo-from-clipboard/?utm_source=pocket_mylist
#+begin_src elisp
