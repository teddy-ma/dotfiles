...
#+begin_src emacs-lisp
  ...
  (defun hikarian-create-controller()
    ...
    (let ((name (read-string "Enter controller name:")))
      (message "you want to create controller %s" name)
      (let* (
             (name_with_level (split-string name "/"))
             (namespace (string-join (butlast name_with_level) "/"))
             (controller_name (car (last name_with_level)))
             )
        ;;(message " %s %s %s" name_with_level namespace controller_name)
        (hikarian-find-or-create-file
         (concat "app/controllers/" namespace)
         (concat controller_name "s_controller.rb")
         ))))
  ...
  (defun hikarian-find-or-create-file (folder file)
    "if file exist open it else create"
    (message "result is %s %s" folder file)
    (let ((project-path-file (concat (hikarian-get-project-root) "/" folder "/" file)))
      ...
      (if (file-exists-p project-path-file)
        ...
        (hikarian-create-and-init-file project-path-file)
        )))

  (defun hikarian-create-and-init-file (file)

    (message "will create file with some content .....") ;; app/controllers/xxx.rb
    (find-file project-path-file)

    (setq hikarian-resource-name "user")
    (insert (yas-expand-snippet (yas-lookup-snippet "hikarian_controller")))
    )
  ...
  (defun hikarian-capitalize-single-resource-name ()
    (capitalize hikarian-resource-name))

  (defun hikarian-pluralize-resource-name ()
    (inflection-pluralize-string hikarian-resource-name))

  (defun hikarian-capitalize-pluralize-resource-name ()
    (capitalize (inflection-pluralize-string hikarian-resource-name)))
