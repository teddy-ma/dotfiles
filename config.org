...
#+begin_src emacs-lisp
  ...
  (defun hikarian-find-or-create-file (folder file)
    ...
    (let ((file_project_path (concat (hikarian-get-project-root) "/" folder "/" file)))
      (message "will check %s" file_project_path)
      (message "is file exists? %s" (file-exists-p file_project_path))
      (if (file-exists-p file_project_path)
          (find-file file_project_path)
        (hikarian-create-and-init-file file_project_path)
        )))

  (defun hikarian-create-and-init-file (file)
    (message "will create file with some content .....")
    (find-file file_project_path)

    (setq resource_name "user")
    (insert (yas-expand-snippet (yas-lookup-snippet "hikarian_controller")))
    )

  (defun get_resource_name ()
    resource_name
    )
  ;;(get_resource_name)
  (provide 'hikarian)
